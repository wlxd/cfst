...
LOCATION_TO_DOMAIN = {
    # 示例映射（可根据实际需求调整）
    # 美国
    "🇺🇸SJC": "sjcv6.616049.xyz",  # 圣何塞
    "🇺🇸LAX": "laxv6.616049.xyz",  # 洛杉矶
    "🇺🇸SEA": "seav6.616049.xyz",  # 西雅图
    "🇺🇸JFK": "jfkv6.616049.xyz",  # 纽约 - 肯尼迪国际机场
    "🇺🇸ORD": "ordv6.616049.xyz",  # 芝加哥 - 奥黑尔国际机场
    ...
    # 日本
    "🇯🇵NRT": "nrtv6.616049.xyz",  # 东京成田
    "🇯🇵HND": "hndv6.616049.xyz",  # 东京羽田
    "🇯🇵JP": "jpv6.616049.xyz",  # 日本

    # 香港
    "🇭🇰HKG": "hkgv6.616049.xyz",  # 香港国际机场
    "🇭🇰HK": "hkv6.616049.xyz",  # 香港

    # 韩国
    "🇰🇷ICN": "icnv6.616049.xyz",  # 仁川国际机场
    "🇰🇷KR": "krv6.616049.xyz",  # 韩国

    # 台湾
    "🇹🇼TPE": "tpev6.616049.xyz",  # 台北桃园机场
    "🇹🇼KHH": "khhv6.616049.xyz",  # 台湾高雄
    "🇹🇼TW": "twv6.616049.xyz",  # 台湾

    # 新加坡
    "🇸🇬SIN": "sinv6.616049.xyz",   # 樟宜机场
    "🇸🇬SG": "sgv6.616049.xyz",  # 新加坡

    # 芬兰
    "🇫🇮HEL": "helv6.616049.xyz",  # 赫尔辛基
    "🇫🇮FI": "fiv6.616049.xyz",  # 芬兰

    # 波兰
    "🇵🇱WAW": "wawv6.616049.xyz",  # 华沙
...
# 从 ipv6.txt 文件中读取前200个 IP 和标记
def get_ips_from_file(file_path, limit=200):
    ip_data = []
    try:
        with open(file_path, "r") as file:
            for line in file:
                # 使用正则表达式提取 IPv6 地址和标记部分
                match = re.match(r'\[([^\]]+)\]#([^┃]+)', line.strip())
                if match:
                    ip = match.group(1)  # 提取 IPv6 地址
                    location = match.group(2).strip()  # 提取标记部分并去除前后空格
                    ip_data.append((ip, location))
...
def add_dns_records_bulk(ip_data):
    ...
    for ip, location in ip_data:
        ...
        if domain:
            ...
            prefix = ".".join(domain.split(".")[:1])  # 提取前两部分（usv6）
...
if __name__ == "__main__":
    # 添加新的 DNS 记录
    ip_data = get_ips_from_file("speed/cfipv6.txt")
    if not ip_data:
        logging.error("未读取到 IP 数据，请检查 ipv6.txt 文件格式是否正确。")
    else:

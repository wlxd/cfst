...
def main():
    ...
    try:
        ...
        # 修改调用 autoddns.py 的部分
        if not no_ddns:  # 只有未指定--no-ddns时才执行
            logging.info("正在调用 autoddns.py 更新 DNS 记录...")
            try:
                subprocess.run([sys.executable, "ddns/autoddns.py"], check=True)
                logging.info("autoddns.py 执行完成。")
            except subprocess.CalledProcessError as e:
                logging.error(f"执行 autoddns.py 失败: {e}")
                sys.exit(1)
